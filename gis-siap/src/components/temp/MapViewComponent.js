...
import { getPetakUser } from "../actions/petakActions";
...
const MapRegister = () => {
  ...
  useEffect(() => {
    dispatch(getPetakUser('1234'));
  }, [dispatch]);

  const listPetak = useSelector((state) => state.petak.petakobj);

  console.log(listPetak);
  ...
  useEffect(() => {
    ...
    polygonLayerRef.current = new VectorTileLayer({
      source: new VectorTileSource({
        format: new MVT(),
        url: `${process.env.REACT_APP_TILE_URL}/function_zxy_id_petakuser/{z}/{x}/{y}?id=1234`,
...
mapInstance.current.on('click', (e) => {
  mapInstance.current.forEachFeatureAtPixel(e.pixel, (feature) => {
    try {
      ...
      const geometryGeoJSON = geojsonFormat.writeGeometryObject(geometry);
