export const CHECK_AUTH = 'CHECK_AUTH';
export const AUTH_ERROR = 'AUTH_ERROR';

//PETAK
export const CREATE_PETAK = "CREATE_PETAK";
export const GET_PETAK = "GET_PETAK";
export const GET_PETAK_USER = "GET_PETAK_USER";
export const GET_PETAK_ID = "GET_PETAK_ID";
export const GET_PETAK_KLAIM_ID = "GET_PETAK_KLAIM_ID";
export const UPDATE_PETAK = "UPDATE_PETAK";
export const DELETE_PETAK = "DELETE_PETAK";

//KLAIM
export const CREATE_KLAIM = "CREATE_KLAIM";
export const GET_KLAIM = "GET_KLAIM";
export const GET_KLAIM_USER = "GET_KLAIM_USER";
export const GET_KLAIM_ID = "GET_KLAIM_ID";
export const UPDATE_KLAIM = "UPDATE_KLAIM";
export const DELETE_KLAIM = "DELETE_KLAIM";
    
//Anggota
export const GET_ANGGOTA = "GET_ANGGOTA";   
export const GET_ANGGOTA_KLAIM = "GET_ANGGOTA_KLAIM";   
export const GET_ANGGOTA_DISETUJUI = "GET_ANGGOTA_DISETUJUI";

//ANALISIS
export const GET_TANAM_PETAK = "GET_TANAM_PETAK";
export const GET_NDPI_ANALISIS = "GET_NDPI_ANALISIS";
export const GET_WATER_ANALISIS = "GET_WATER_ANALISIS";
export const GET_BARE_ANALISIS = "GET_BARE_ANALISIS";